...
HORA=int(os.environ.get('COACH_HORA','8')); MINUTO=int(os.environ.get('COACH_MINUTO','0'))
...
"""
    ...
    if bajos:
        s+="\n🔥 *FOCO DE GESTIÓN*\n"
        for i,x in enumerate(bajos,1):
            s+=f"{i}️⃣ {x['nombre'].title()}\n"
            s+=f"   {x['avance']*100:.1f}% | Falta S/. {x['falta']:,.2f}\n"

        s+="\n🎯 ACCIÓN DEL JEFE\n"
        s+="Priorizar seguimiento a estos 3 vendedores y revisar su plan de recuperación para los días laborables restantes.\n"
...
if __name__=='__main__':
    logger.info('🧠 COACH COMERCIAL WORKER V10 | %02d:%02d America/Lima',HORA,MINUTO)
    logger.info('BD=%s | VENDEDORES=%s | FERIADOS=%s',BD,VENDEDORES_XLSX,FERIADOS_XLSX)
    logger.info('⏰ Horario automático: %02d:%02d America/Lima | Laborables: lunes-sábado, excluye feriados',HORA,MINUTO)
