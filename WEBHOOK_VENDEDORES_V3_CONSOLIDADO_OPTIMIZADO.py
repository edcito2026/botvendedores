...
# -*- coding: utf-8 -*-
"""
WEBHOOK RECEPCIÓN DE MENSAJES WHATSAPP - V3 CONSOLIDADO - OPTIMIZADO
🔧 OPTIMIZACIONES:
1. Removida importación `re` (no utilizada)
2. Cambio de búsqueda por `Cliente = Raz_Social` a `Cod_Clie` con CAST (más preciso)
3. Agregado `finally` en gestión de conexiones DB para garantizar cierre
...
# ============================================================
# 6. OBTENER CLIENTES TROYA - 🔧 OPTIMIZADO
# ============================================================
...
def obtener_clientes_troya(nombre_vendedor):
    ...
    try:
        ...
        for cliente in clientes:
            try:
                # Ventas mes actual - ARCOR ONLY - Búsqueda por Cod_Clie ✅
                cursor.execute("""
                    SELECT ROUND(SUM(CAST(Imp_Total AS REAL)), 2) as total
                    FROM VENTAS2026
                    WHERE CAST(Cod_Clie AS REAL) = ? AND Vendedor = ? AND Periodo = ? AND Proveedor = 'ARCOR'
                """, (float(cliente['Cod_Clie']), nombre_vendedor, periodo_actual))
                ...
                # Ventas mes anterior - ARCOR ONLY - Búsqueda por Cod_Clie ✅
                cursor.execute("""
                    SELECT ROUND(SUM(CAST(Imp_Total AS REAL)), 2) as total
                    FROM VENTAS2026
                    WHERE CAST(Cod_Clie AS REAL) = ? AND Vendedor = ? AND Periodo = ? AND Proveedor = 'ARCOR'
                """, (float(cliente['Cod_Clie']), nombre_vendedor, periodo_anterior))
...
def obtener_clientes_troya_generales():
    ...
    try:
        ...
        # Verificar compras - Búsqueda por Cod_Clie ✅
        for cliente in clientes:
            try:
                cursor.execute("""
                    ...
                    FROM VENTAS2026
                    WHERE CAST(Cod_Clie AS REAL) = ? AND Vendedor = ? AND Periodo = ? AND Proveedor = 'ARCOR'
                """, (float(cliente['Cod_Clie']), cliente['Vendedor'], periodo))
...
def status():
    ...
    return jsonify({
        ...
        "webhook": "/webhook",
        "version": "V3 Consolidado - OPTIMIZADO",
        "periodo": ctx["periodo"],
...
if __name__ == "__main__":
    logger.info("=" * 70)
    logger.info("🚀 WEBHOOK V3 CONSOLIDADO - RESUMEN + TROYA (OPTIMIZADO)")
    logger.info("=" * 70)
    ctx = obtener_contexto_periodo()
    dias = calcular_dias_laborables_periodo()
    ...
    logger.info("   1. Removida importación `re` (sin usar)")
    logger.info("   2. Búsqueda por Cod_Clie (en lugar de Raz_Social)")
    logger.info("   3. Agregado `finally` en gestión de conexiones DB")
    logger.info("=" * 70)
